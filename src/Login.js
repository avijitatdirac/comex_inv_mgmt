...
class Login extends Component {
  ...
  handleSubmit(e) {
    ...
    if (!username) {
      this.setState({
        usernameError: true,
        usernameErrorText: validation.messages().emailEmpty
      });
      isValid = false;
    }
    if (validation.validEmail(username)) {
      ...
      this.setState({
        usernameError: true,
        usernameErrorText: validation.messages().notValidEmail
      });
      isValid = false;
    }
    if (password) {
      if (!validation.passwordLenghtCheck(password)) {
        ...
        this.setState({
          passwordError: true,
          passwordErrorText: validation.messages().passwordLength
        });
      }
    } else {
      this.setState({
        passwordError: true,
        passwordErrorText: validation.messages().passEmpty
      });
      isValid = false;
    }
    if (isValid) {
      const endpoint = "/login/check_login";
      ...
      fetchAPI(endpoint, data)
        ...
        .then(data => {
          if (data.is_success) {
            ...
            this.setState({
              passwordError: true,
              passwordErrorText: validation.messages().invalidUser
            });
          }
        });
      ...
      <div className="login-form">
        ...
        >
          <Grid.Column style={{ maxWidth: 450 }}>
            ...
            {submitted &&
              usernameErrorText && (
                <Message error header="Error" content={usernameErrorText} />
              )}

            {submitted &&
              passwordErrorText && (
                <Message error header="Error" content={passwordErrorText} />
              )}
            {submitted &&
              successText && (
                <Message success header="Success" content={successText} />
              )}
          </Grid.Column>
        </Grid>
      </div>
