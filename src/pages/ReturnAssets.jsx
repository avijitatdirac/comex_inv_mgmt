...
class ReturnAssets extends Component {
	...
	callDamagedData = () => {
		fetch(`/damaged_assets`, { method: 'GET' })
			.then(r => r.json())
			.then(data => {
	...
	callModifiedData = () => {
		fetch(`/get_asset_config`, { method: 'GET' })
			.then(r => r.json())
			.then(data => {
		...
		this.state.damageDetails.forEach(damage => {
			if (damage.serialnumber === this.state.selectedDamageId) {
				this.setState(
					...
					console.log(
						this.state.damageId + ' ' + this.state.damageSerialNumber + ' ' + this.state.damageStatus
					)
				);
			}
		});
	...
	getAllData = () => {
		...
		fetch(`/get_customer_order_details?customer_id=${this.state.selectedCustomerId}`, { method: 'GET' })
			.then(r => r.json())
			.then(data => {
				// parsing json data
				//console.log(data)
				...
				for (var i = 0; i < dynadata.length; i++) {
					if (
						...
						if (dynadata[i].order_date === '0000-00-00 00:00:00') var madate = '';
						if (dynadata[i].rental_end_date === '0000-00-00 00:00:00') var purdate = '';
					} else {
	...
	removeConfigState = () => {
		...
		fetch(`/change_config_status?id=${this.state.configTableID}`, { method: 'GET' })
			.then(r => r.json())
			.then(data => {
				this.setState(
					{
	...
	sendForRepair = () => {
		...
		fetch(`/send_for_repair?id=${this.state.damageId}&comments=${this.state.damageComments}`, { method: 'GET' })
			.then(r => r.json())
			.then(data => {
				this.setState(
					{
	...
	returnFromRepair = () => {
		// console.log(this.state.damageId)
		fetch(`/return_from_repair?id=${this.state.damageId}`, { method: 'GET' })
			.then(r => r.json())
			.then(data => {
				this.setState(
					{
						...
						<Table.Row>
							...
							<Table.Cell>
								{this.state.modifiedButtonRender ? (
									<Button width="100px" color="red" label="Remove" onClick={this.removeConfigState}>
		...
		return (
			<div>
				<Dimmer.Dimmable as={Segment} dimmed={dimmerActive}>
					...
					<Table color="teal" striped>
                        ...
                        onClick={this.checkedAssetsProper.bind(this,obj.assetid,obj)}/>*/}
                            ...
                            onClick={this.checkedAssetsDamaged.bind(this,obj.assetid,obj)}/>*/}
										...
										{<Form.Field>										
											<Radio key={obj.unique+'that'}
												label=""
												name={obj.unique}
												value={obj.unique+'that'}
												checked={this.state[obj.unique] === obj.unique+'that'}
									...
									<Table.Cell>
										<Form.Input
											value={obj.comments}
											onChange={this.changeComments.bind(this, obj.assetid, obj)}
										/>
									</Table.Cell>
								</Table.Row>
							))}
					...
					<div>
						<center>
							<Button width="100px" color="blue" onClick={this.spliceNullCheck} label="Return">
	...
	submitData = () => {
		...
		fetch(
			`/change_status_on_return?data=${JSON.stringify(this.state.selectedcheckboxasset)}&oid=${JSON.stringify(
			...
			<Segment>
				<div style={{}}>
					...
					<Dropdown
						...
						onChange={(e, data) =>
							this.setState({ selectedCustomerId: data.value }, this.populateCustomerData)
						}
	...
	selectedAssetType = (event, d) => {
		...
		let tableData = data.filter(obj => obj.assettype.toLowerCase().indexOf(value.toLowerCase()) !== -1);
			...
			<div className="field">
				...
				<Form key="renderCategoryDropdown">
					<Form.Group widths="equal">
						<Form.Select
							onChange={(e, data) =>
								this.setState({ selectedCategory: data.value }, this.selectedAssetType(e, data))
							}
	...
	filterResult = () => {
		...
		let data = this.state.tableDataInitial.slice();
		let tableData = data.filter(obj => obj.serialnumber.toLowerCase().indexOf(value.toLowerCase()) !== -1);
		this.setState({ tableData });
	};
