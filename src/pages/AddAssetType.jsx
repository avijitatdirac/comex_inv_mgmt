...
import { Segment, Form, Button, Icon, Divider } from "semantic-ui-react";
import { Input, Table, Dimmer, Loader } from "semantic-ui-react";
...
export default class AddAssetType extends React.Component {
  ...
  handleAssetNameSubmit = () => {
    ...
    if (!this.state.assetTypeName || this.state.assetTypeName === "") {
      ...
      aname = str.replace(/\w\S*/g, function(txt) {
    ...
    this.setState(
      {
        ...
        assetTypeNameError: errorassettype
      },
      this.commitassettypename
    );
  };
  ...
  handleAttributeSubmit = () => {
    ...
    if (this.state.assetTypeAttributes[l].name === "") {
      ...
      fetchAPI("/asset/insert_asset_type", {
        type_name: this.state.assetTypeName,
        attributes: this.state.assetTypeAttributes
      })
        .then(r => r.json())
        .then(data => {
          this.setState({
    ...
    if (this.state.assetTypeAttributes[idx].name === "") {
      ...
      this.setState({
        assetTypeAttributes: this.state.assetTypeAttributes.concat([
          { name: "", isMandatory: true, isModifiable: true }
        ])
      });
    }
  };
  ...
  handleRemoveAttribute = idx => () => {
    if (this.state.assetTypeAttributes.length !== 1)
      this.setState({
        assetTypeAttributes: this.state.assetTypeAttributes.filter(
          (s, sidx) => idx !== sidx
        )
      });
  };
  ...
  onChangeAssetTypeName = event => {
    ...
    var aname = str.replace(/\w\S*/g, function(txt) {
      return txt.charAt(0).toUpperCase() + txt.substr(1).toLowerCase();
    });
    ...
    return (
      <div>
        ...
        <Table color="teal" striped>
          ...
          <Table.Footer>
            <Table.Cell>
              <Form.Button onClick={this.handleAttributeDimmer} color="blue">
          ...
          <Segment>
            <Form>
              <Form.Input
                ...
                width={10}
                onChange={this.onChangeAssetTypeName}
              />
              {!this.state.isAssetTypeSubmitted ? (
                <Form.Button onClick={this.handleAssetNameSubmit} basic>
                  <Icon name="tv" />Create
                </Form.Button>
              ) : (
                undefined
            ...
            {this.state.isAssetTypeSubmitted && !this.state.assetTypeNameError
              ? this.renderAssetTypeAttributesForm()
              : undefined}
          </Segment>
        </Dimmer.Dimmable>
      </div>
