...
class GenerateChallan extends React.Component {
  ...
  calculateTotalGST = () => {
    ...
    this.state.challanCartItems.forEach(element => {
      l_gst =
        (Number(element.gst !== undefined ? element.gst : 0) *
          Number(
            element.totalUnitPrice !== undefined ? element.totalUnitPrice : 0
          )) /
        100;
  ...
  calculateCurrentPrice = obj => {
    ...
    var r;
    if (purchaseMonth >= 5 && purchaseMonth <= 9) r = 40;
    else r = 20;
      ...
      fetchAPI("/challan/insert_challan_draft", {
        ...
        .then(data => {
          if (data.isSuccess) {
            ...
            fetchAPI("/asset/change_inventory_status", {
              data: cartAssetIds
            })
              .then(r => r.json())
              .then(data => {
  ...
  cancelDraft = () => {
    ...
    fetchAPI("/challan/delete_challan_details", {
      challan_id: this.props.selectedDraftId
    })
      .then(r => r.json())
      .then(data => {
        if (data.isSuccess) {
  ...
  submitChallan = () => {
    ...
    // else set status to 0 in db
      ...
      fetchAPI("/order/order_create", { data: jsonRequestData })
        .then(result => result.json())
        .then(data => {
          if (data.isSuccess) {
            console.log("order insertion success");
            // 2. insert into asset config
            fetchAPI("/config/change_config_table_on_add", {
              data: jsonChallanCartItems
            })
              .then(r => r.json())
              .then(data => {
                if (data.isSuccess) {
                  ...
                  // else if its order page submission
                    ...
                    fetchAPI("/asset/change_inventory_status", {
                      ...
                      .then(data => {
                        if (data.isSuccess) {
                          notify.success(
                            "submitted after change_inventory_status"
                          );
                          this.setState({ printChallan: true });
                          console.log(
                            "last_challan_number =" +
                              JSON.stringify(
                                data.last_challan_number[0].last_challan
                              )
                          );
                          this.setState({
                            last_challan_number:
  ...
  onChangeTaxType = (evt, data) =>
    this.setState({ selectedTaxType: data.value });
  ...
  onChangeUnitPrice = idx => event => {
    ...
    var rentStartDate =
      this.state.rentStartDate !== undefined
        ? moment(this.state.rentStartDate)
        : moment();
    const newCartItems = temp.map((item, sidx) => {
      if (idx !== sidx) {
        ...
        var rentEndDate =
          item.rentEndDate !== undefined ? moment(item.rentEndDate) : moment();

        var multiplier =
          this.state.selectedPaymentMode === "Daily"
            ? rentEndDate.diff(rentStartDate, "days")
            : 1;
        ...
        var total =
          Number(totalUnitPrice) + (Number(totalUnitPrice) * Number(gst)) / 100;
        return {
          ...item,
          unitPrice: unitPrice,
  ...
  onChangeRentEndDate = idx => date => {
    ...
    const newCartItems = temp.map((item, sidx) => {
      if (idx !== sidx) {
        ...
        var totalUnitPrice =
          (item.unitPrice !== undefined ? item.unitPrice : 0) * days;
        var total =
          Number(totalUnitPrice) + (Number(gst) * Number(totalUnitPrice)) / 100;
        return {
          ...item,
  ...
  ueaNumberOnChangeHandler = event =>
    this.setState({ ueaNumber: event.target.value });
  poOnChangeHandler = event => this.setState({ po: event.target.value });
  poReferenceOnChangeHandler = event =>
    this.setState({ poReference: event.target.value });
  cnNumberOnChangeHandler = event =>
    this.setState({ cnNumber: event.target.value });
  deliveryPersonOnChangeHandler = event =>
  ...
  onChangeRentStartDate = date => {
    ...
    this.setState({
      challanCartItems: newCartItems,
      rentStartDate: moment(date)
    });
  };
      ...
      return [
        ...
        <Segment>
          ...
          >
            ...
            this.state.selectedChallanType !== null ? (
        ...
        <Table.Body>
          {this.state.challanCartItems.map((obj, idx) => (
            <Table.Row key={obj.id}>
              ...
              <Table.Cell>
                <Input value={obj.gst} onChange={this.onChangeGst(idx)} small />
              </Table.Cell>
              <Table.Cell>
                {" "}
                {"   "}
