...
export default class AddAsset extends React.Component {
    ...
    fetch(`/get_asset`, { method: "GET" })
      ...
      .then(data => {
        ...
        for (i = 0; i < r.results.length; i++) {
          ...
          this.setState({
            assetTypes: this.state.assetTypes.concat([
              {
                key: r.results[i].id,
                id: r.results[i].id,
  ...
  onChangeWarehouseLocation = event =>
  ...
  onChangeSupplierInvoiceNo = event =>
    this.setState({ supplierInvoiceNo: event.target.value });
  onChangeHsnCode = event => this.setState({ hsnCode: event.target.value });

  onChangeTransferOrder = event =>
  ...
  onAttributeValueChange = idx => evt => {
    const newAttribute = this.state.assetTypeAttributes.map(
      (attribute, sidx) => {
      ...
      <Form>
        ...
        <Form.Group>
          ...
          <Form.Input label="Transfer Order Date.">
            <DatePicker
              dateFormat="DD/MM/YYYY"
              selected={this.state.transferOrderDate}
        ...
        <Form.Group>
          <Form.Input
            onChange={this.onChangeWarehouseLocation}
        ...
        <Form.Group>
          <Form.Input label="Procurement Date" placeholder="Procurement Date">
            <DatePicker
              dateFormat="DD/MM/YYYY"
              selected={this.state.procurementDate}
              onChange={this.onChangeProcurementDate}
            />
          </Form.Input>
          <Form.Input
            onChange={this.onChangePartCode}
            label="Part Code"
        ...
        <Form.Group>
          ...
          <Form.Input label="Warranty end date">
            <DatePicker
              dateFormat="DD/MM/YYYY"
              selected={this.state.warrantyEndDate}
              onChange={this.onChangewarrantyEndDate}
            />
          </Form.Input>
          {/* <Form>
        ...
        <Form.Group>
          ...
          />
          <Form.Input
            onChange={this.onChangeSupplierInvoiceNo}
          ...
          <Form.Input label="Supplier invoice date">
            <DatePicker
              dateFormat="DD/MM/YYYY"
              selected={this.state.supplierInvoiceDate}
              onChange={this.onChangeSupplierInvoiceDate}
            />
          </Form.Input>
        </Form.Group>
        <Form.Group>
          <Form.Input
            onChange={this.onChangeTransferOrder}
            width={14}
    ...
    return (
      <div>
        <Form.Input
          ...
          onChange={(e, data) =>
            this.updateState("enteredSerialNo", data.value)
          }
        ...
        <div>
          {this.state.serialNos.map((val, idx) => (
            <Label key={idx} style={{ margin: "2px" }} color="green">
              ...
              <Icon
                name="delete"
                onClick={this.removeSerialNo.bind(this, idx)}
              />
            </Label>
          ))}
        </div>
  ...
  onAssetSubmit = () => {
    ...
    this.state.serialNos.forEach(serialNo => {
      ...
      let type_name = this.state.selectedAssetType;
      let staticAttr = {
        serialNo: serialNo,
      ...
      let dynamic = this.state.assetTypeAttributes;
        ...
        <Segment color="blue">
          <Form>
            <Form.Group widths="equal">
              ...
              <Button
                ...
                labelPosition="right"
                onClick={() => {
                  this.props.history.push("/AddAssetType");
                }}
              >
            ...
            <Form.Button color="blue" onClick={this.onAssetSubmit}>
              <Icon name="save" />Save Asset
            </Form.Button>
          </Form>
          {this.state.submitSuccess ? (
            <Message
              success
              header="Success!"
              content="Your asset has been submitted."
            />
          ) : this.state.submitFailure ? (
            <Message
              error
              header="Failure!"
              content="Could not submit asset."
            />
          ) : (
            undefined
          )}
